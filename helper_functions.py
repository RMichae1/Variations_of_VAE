...
def positional_splitter(assay_df, query_seqs, val=True, aln_path=None, offset = 4, pos_per_fold = 100, 
    ...
    for i in range(len(np.unique(mut_pos))//(pos_per_fold)+1):  
        ...
        if val:
            print("VAL STEP:")
            existing_mutations = train_mut + test_mut
            val_mut = np.setdiff1d(unique_mut, existing_mutations)
            print(val_mut)
        else:
        ...
        if offset > 0:
            ...
            buffer_mut = np.setdiff1d(unique_mut, np.concatenate([train_mut, test_mut, val_mut]))
        else:
            buffer_mut = []
